...
c     CR mixing matrix definition - v(i)*zr(i,j)
      ...
      double complex function v_nns(i,j,k)
c     Scalar - 2 neutralino left vertex
      implicit double precision (a-h,o-z)
      double complex zn
      common/hmass/cm(2),rm(2),pm(2),zr(2,2),zh(2,2)
     ...
     1      + (zr(1,k)*zn(3,j)-zr(2,k)*zn(4,j))*(zn(1,i)*st-zn(2,i)*ct))
      ...
      double complex function v_ccs(i,j,k)
c     Scalar - 2 chargino left vertex
      implicit double precision (a-h,o-z)
...
c     Scalar - 2 slepton vertex initialization
     ...
     $        + e2/4/sct2*br(k)*(1 - 4*st2)*dconjg(zl(l,i))*zl(l,j)
        vi_lls = vi_lls - v1*yl(l)**2*zr(1,k)
     $       *(dconjg(zl(l,i))*zl(l,j) + dconjg(zl(l+3,i))*zl(l+3,j))
        vi_lls = vi_lls - yl(l)/sq2*zr(2,k)
     $       *(dconjg(h*zl(l,i))*zl(l+3,j) 
     $       + h*zl(l,j)*dconjg(zl(l+3,i)))
        do 10 m=1,3
          vi_lls = vi_lls - zr(1,k)/sq2
     1          *(dconjg(ls(l,m)*zl(m+3,i))*zl(l,j)
     2          + ls(l,m)*zl(m+3,j)*dconjg(zl(l,i)))
10        vi_lls = vi_lls + zr(2,k)/sq2
     1          *(dconjg(ks(l,m)*zl(m+3,i))*zl(l,j)
     2          + ks(l,m)*zl(m+3,j)*dconjg(zl(l,i)))
      return
      end
...
c     Scalar - 2 d-squark vertex initialization
      implicit double precision (a-h,o-z)
      double complex zd,zu,h,ls,ks,ds,es,us,ws
      common/vpar/st,ct,st2,ct2,sct,sct2,e,e2,alpha,wm,wm2,zm,zm2,pi,sq2
      common/hpar/hm1,hm2,hs,h
      common/soft/ls(3,3),ks(3,3),ds(3,3),es(3,3),us(3,3),ws(3,3)
      common/hmass/cm(2),rm(2),pm(2),zr(2,2),zh(2,2)
     ...
     $        + e2/12/sct2*br(k)*(3 - 4*st2)*dconjg(zd(l,i))*zd(l,j)
        vi_dds = vi_dds - v1*yd(l)**2*zr(1,k)
     $       *(dconjg(zd(l,i))*zd(l,j) + dconjg(zd(l+3,i))*zd(l+3,j))
        vi_dds = vi_dds - yd(l)/sq2*zr(2,k)
     $       *(dconjg(h*zd(l,i))*zd(l+3,j) 
     $       + h*zd(l,j)*dconjg(zd(l+3,i)))
        do 10 m=1,3
          vi_dds = vi_dds - zr(1,k)/sq2
     1          *(dconjg(ds(l,m)*zd(m+3,i))*zd(l,j)
     2          + ds(l,m)*zd(m+3,j)*dconjg(zd(l,i)))
10        vi_dds = vi_dds + zr(2,k)/sq2
     1          *(dconjg(es(l,m)*zd(m+3,i))*zd(l,j)
     2          + es(l,m)*zd(m+3,j)*dconjg(zd(l,i)))
      return
      end
...
c     Scalar - 2 u-squark vertex initialization
     ...
     $        - e2/12/sct2*br(k)*(3 - 8*st2)*dconjg(zu(l,i))*zu(l,j)
        vi_uus = vi_uus - v2*yu(l)**2*zr(2,k)
     $       * (dconjg(zu(l,i))*zu(l,j) + dconjg(zu(l+3,i))*zu(l+3,j))
        vi_uus = vi_uus + yu(l)/sq2*zr(1,k)
     $       * (dconjg(h*zu(l+3,i))*zu(l,j) 
     $       + h*zu(l+3,j)*dconjg(zu(l,i)))
        do 10 m=1,3
          vi_uus = vi_uus + zr(2,k)/sq2
     1          *(dconjg(us(l,m)*zu(l,i))*zu(m+3,j)
     2          + us(l,m)*zu(l,j)*dconjg(zu(m+3,i)))
10        vi_uus = vi_uus + zr(1,k)/sq2
     1          *(dconjg(ws(l,m)*zu(l,i))*zu(m+3,j)
     2          + ws(l,m)*zu(l,j)*dconjg(zu(m+3,i)))
      return
      end
...
c     2 scalar - 2 slepton vertex
     ...
     1         *dconjg(zl(m,k))*zl(m,l)
10      v_llss = v_llss - yl(m)**2*zr(1,i)*zr(1,j)
     1        *(dconjg(zl(m,k))*zl(m,l) + dconjg(zl(m+3,k))*zl(m+3,l))
      return
      end
...
c     2 scalar - 2 U-squark vertex
     ...
     1         *dconjg(zu(m,k))*zu(m,l)
10      v_uuss = v_uuss - yu(m)**2*zr(2,i)*zr(2,j)
     1        *(dconjg(zu(m,k))*zu(m,l) + dconjg(zu(m+3,k))*zu(m+3,l))
      return
      end
...
c     2 scalar - 2 D-squark vertex
     ...
     1         *dconjg(zd(m,k))*zd(m,l)
10      v_ddss = v_ddss - yd(m)**2*zr(1,i)*zr(1,j)
     1        *(dconjg(zd(m,k))*zd(m,l) + dconjg(zd(m+3,k))*zd(m+3,l))
      return
      end
...
c     Pseudoscalar - 2 slepton vertex
     ...
     1          - ls(l,m)*zl(m+3,j)*dconjg(zl(l,i)))/sq2
10        vi_llp = vi_llp - zh(2,k)*(dconjg(ks(l,m)*zl(m+3,i))*zl(l,j)
     1          - ks(l,m)*zl(m+3,j)*dconjg(zl(l,i)))/sq2
      return
      end
...
c     Pseudoscalar - 2 D-squark vertex
     ...
     1          - ds(l,m)*zd(m+3,j)*dconjg(zd(l,i)))/sq2
 10       vi_ddp = vi_ddp - zh(2,k)*(dconjg(es(l,m)*zd(m+3,i))*zd(l,j)
     1          - es(l,m)*zd(m+3,j)*dconjg(zd(l,i)))/sq2
      return
      end
...
c     Pseudoscalar - 2 U-squark vertex
     ...
     1          - us(l,m)*zu(l,j)*dconjg(zu(m+3,i)))/sq2
 10       vi_uup = vi_uup - zh(1,k)*(dconjg(ws(l,m)*zu(l,i))*zu(m+3,j)
     1         - ws(l,m)*zu(l,j)*dconjg(zu(m+3,i)))/sq2
      return
      end
...
c     2 pseudoscalars - 2 sleptons vertex
     ...
     1        * dconjg(zl(m,k))*zl(m,l)
10      v_llpp = v_llpp - yl(m)**2*zh(1,i)*zh(1,j)
     1        *(dconjg(zl(m,k))*zl(m,l) + dconjg(zl(m+3,k))*zl(m+3,l))
      return
      end
...
c     2 pseudoscalars - 2 U-squarks vertex
     ...
     1        * dconjg(zu(m,k))*zu(m,l)
10      v_uupp = v_uupp - yu(m)**2*zh(2,i)*zh(2,j)
     1        *(dconjg(zu(m,k))*zu(m,l) + dconjg(zu(m+3,k))*zu(m+3,l))
      return
      end
...
c     2 pseudoscalars - 2 D-squarks vertex
     ...
     1         *dconjg(zd(m,k))*zd(m,l)
10      v_ddpp = v_ddpp - yd(m)**2*zh(1,i)*zh(1,j)
     1        *(dconjg(zd(m,k))*zd(m,l)+dconjg(zd(m+3,k))*zd(m+3,l))
      return
      end
...
c     Charged Higgs - u-squark - d-squark vertex initialization
      implicit double precision (a-h,o-z)
      double complex zd,zu,h,ls,ks,ds,es,us,ws,ckm
      common/vpar/st,ct,st2,ct2,sct,sct2,e,e2,alpha,wm,wm2,zm,zm2,pi,sq2
      common/hpar/hm1,hm2,hs,h
      common/soft/ls(3,3),ks(3,3),ds(3,3),es(3,3),us(3,3),ws(3,3)
      common/hmass/cm(2),rm(2),pm(2),zr(2,2),zh(2,2)
...
c     2 charged Higgs - 2 D-squark vertex initialization
     ...
     1        * dconjg(zd(m,i))*zd(m,j)
     2        - yd(m)**2*zh(1,k)*zh(1,l)*dconjg(zd(m+3,i))*zd(m+3,j)
        do 10 n=1,3
          do 10 o=1,3  
 10         vi_ddhh = vi_ddhh - yu(n)**2*zh(2,k)*zh(2,l)
     1        * ckm(m,n)*zd(m,j)*dconjg(ckm(o,n)*zd(o,i))
      return
      end
...
c     2 charged Higgs - 2 u-squark vertex initialization
     ...
     1        * dconjg(zu(m,i))*zu(m,j)
     2        - yu(m)**2*zh(2,k)*zh(2,l)*dconjg(zu(m+3,i))*zu(m+3,j)
        do 10 n=1,3
          do 10 o=1,3  
 10         vi_uuhh = vi_uuhh - yd(m)**2*zh(1,k)*zh(1,l)
     1        * ckm(m,n)*zu(n,j)*dconjg(ckm(m,o)*zu(o,i))
      return
      end
...
c     2 charged Higgs - 2 u-squark vertex initialization
      ...
      do 10 m=1,3
       do 10 n=1,3
 10      vi_udhs = vi_udhs + (zu(n,i)*zd(m,j)*(yu(n)**2*zh(2,k)*zr(2,l)
