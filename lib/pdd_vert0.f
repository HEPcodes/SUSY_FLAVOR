...
c     FILENAME: PDD_VERT0.F
c     Released: 21:09:2008(J.R.)

ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     This file contains neutral pseudoscalar Higgs - down quark-down c
c     quark vertex formfactors at vanishing external momenta          c
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine pdd_vert0_uwh(i,j,k,form)
c     Up quark-W-charged higgs in loop
      ...
      do l=1,3
         tmp = e2/st2/2/sq2*zh(1,k)*dconjg(ckm(j,l))*ckm(i,l)
     $        * cp1(um(l),wm,cm(k))
      ...
      subroutine pdd_vert0_uuw(i,j,k,form)
c     Up quark-up-quark-W in loop
      ...
      common/yukawa/yl(3),yu(3),yd(3)
      do l=1,3
         tmp = e2/st2/sq2*zh(2,k)*um(l)*yu(l)*dconjg(ckm(j,l))*ckm(i,l)
     $        * cp0(um(l),um(l),wm)
      ...
      subroutine pdd_vert0_uuh(i,j,k,form)
c     Up quark-up-quark-charged higgs in loop
      ...
      do l=1,3
         do m=1,2
            tmp = zh(2,k)*zh(1,m)*zh(2,m)*yu(l)*yu(l)*dconjg(ckm(j,l))
     ...
     $           * cp0(um(l),um(l),cm(m)))
      ...
      subroutine pdd_vert0_hhu(i,j,k,form)
c     Charged higgs-charged higgs-up quark in loop
      ...
      if (k.eq.2) return
      do l=1,3
         tmp = e*wm/2/st*yu(l)*um(l)*dconjg(ckm(j,l))*ckm(i,l)
     $        * cp0(um(l),wm,cm(1))
         form(1) = form(1) - yd(i)*tmp
         form(2) = form(2) + yd(j)*tmp
      end do
      ...
      subroutine pdd_vert0_ccu(i,j,k,form)
c     Up squark-chargino-chargino in loop
      implicit double precision (a-h,o-z)
      double complex form(2),a1,a2
      double complex v_ccp,vl_duc,vr_duc
     ...
     $              + a1*fcm(l)*fcm(m)*cp0(fcm(l),fcm(m),sum(n)))
      ...
      subroutine pdd_vert0_uuc(i,j,k,form)
c     Chargino-up squark-up squark in loop
      implicit double precision (a-h,o-z)
      double complex form(2),cv
      double complex vl_duc,vr_duc,v_uup
      double complex zu,zd,zpos,zneg
      common/charg/fcm(2),zpos(2,2),zneg(2,2)
      common/sqmass/sum(6),sdm(6),zu(6,6),zd(6,6)
      do l=1,6
     ...
     $              * vr_duc(j,l,n)*fcm(n)*cp0(sum(l),sum(m),fcm(n))
      ...
      subroutine pdd_vert0_nnd(i,j,k,form)
c     Down squark-neutralino-neutralino in loop
      implicit double precision (a-h,o-z)
      double complex form(2),vv
      double complex v_nnp,vl_ddn,vr_ddn
     ...
     $              +dconjg(vv)*fnm(l)*fnm(m)*cp0(fnm(l),fnm(m),sdm(n)))
      ...
      subroutine pdd_vert0_ddn(i,j,k,form)
c     Neutralino-down squark- down squark in loop
      implicit double precision (a-h,o-z)
      double complex form(2),cv
      double complex vl_ddn,vr_ddn,v_ddp
      double complex zu,zd,zn
      common/neut/fnm(4),zn(4,4)
      common/sqmass/sum(6),sdm(6),zu(6,6),zd(6,6)
      do l=1,6
...
c     Gluino-down squark- down squark in loop
      ...
      do l=1,6
         do m=1,6
            tmp = - al*v_ddp(l,m,k)*gm1*cp0(sdm(l),sdm(m),gm1)
      ...
      subroutine pdd_vert0(i,j,k,form)
c     Full bare Pdd formfactors at vanishing external momenta
      implicit double precision (a-h,o-z)
      double complex form(2)
      common/vpar/st,ct,st2,ct2,sct,sct2,e,e2,alpha,wm,wm2,zm,zm2,pi,sq2
...
c     Higgs and gauge boson contributions
      if (ih.eq.1) then
         call pdd_vert0_uwh(i,j,k,form)
         call pdd_vert0_uuw(i,j,k,form)
         call pdd_vert0_hhu(i,j,k,form)
         call pdd_vert0_uuh(i,j,k,form)
      end if
c     chargino contributions
      if (ic.eq.1) then
         call pdd_vert0_ccu(i,j,k,form)
         call pdd_vert0_uuc(i,j,k,form)
      end if
c     neutralino contributions
      if (in.eq.1) then
         call pdd_vert0_nnd(i,j,k,form)
         call pdd_vert0_ddn(i,j,k,form)
      end if
c     gluino contributions
      if (ig.eq.1) call pdd_vert0_ddg(i,j,k,form)
      do l=1,2
      ...
      subroutine pdd_vert_eff(i,j,k,form)
c     Full flavour violating Pdd formfactors at vanishing external momenta
c     including reducible contributions
      ...
      common/hmass/cm(2),rm(2),pm(2),zr(2,2),zh(2,2)
      common/vev/v1,v2
      if (i.eq.j) stop 'pdd_vert_eff defined only for i<>j'
      call pdd_vert0(i,j,k,form)
      form(1) = form(1) + zh(1,k)/v1*dsl_sig0(i,j)
      form(2) = form(2) - zh(1,k)/v1*dsr_sig0(i,j)
      end

c      subroutine pdd_ren0(i,j,k,form)
c     Full renormalized Pdd formfactors at vanishing external momenta
